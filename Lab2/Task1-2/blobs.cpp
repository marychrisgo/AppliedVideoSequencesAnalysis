	...
	for(int i=0; i< fgmask.rows;++i){
		for(int j=0; j<fgmask.cols;++j){
				if(fgmask.at<uchar>(i,j)==255){
					IDcount +=1;
					//std::cout << IDcount  << std::endl;
					//Output.at<uchar>(i,j) = IDcount;
					//fgmask.at<uchar>(i,j) = 0; // se puede cambiar a cero o a un valor intermedio
					...
					cv::floodFill(Output,Point(j,i),IDcount, &Selection,cv::Scalar(0),cv::Scalar(0),connectivity);
					// Add ID+BLOB to blob
...
int removeSmallBlobs(std::vector<cvBlob> bloblist_in, std::vector<cvBlob> &bloblist_out, int min_width, int min_height)
{
	...
	for(int i = 0; i < bloblist_in.size(); i++)
	{
		...
		//cout << "Blob Errased # " <<i<<" widht: "<< blob_in.w <<" height: "<< blob_in.h<<endl;
		//bloblist_in.erase(bloblist_in.begin()+i);
		// ...
...
//end distances
 int classifyBlobs(std::vector<cvBlob> &bloblist)
 {
 	...
 	for(int i = 0; i < bloblist.size(); i++)
 	{
 		...
 		// Store again the label
 		//bloblist.push_back(blob); // void implementation (does not remove)
