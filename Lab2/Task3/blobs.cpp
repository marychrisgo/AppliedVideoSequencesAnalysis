...
#define I_COST 5 // to set // increment cost for stationarity detection
#define D_COST 2 // to set // decrement cost for stationarity detection
...
int extractStationaryFG (Mat fgmask, Mat & fgmask_history, Mat & sfgmask)
{
	...
	// eliminating shadow from the mask	
	fgmask = fgmask == 255; 
	
	// converting it to float for uniformity
	fgmask.convertTo(fgmask, CV_32FC1); 

	// formula for getting 'Foreground History Image'
	fgmask_history = I_COST * (fgmask/255) - D_COST * (1 - fgmask/255) + fgmask_history ;

	// converting it to float for uniformity
	fgmask_history.convertTo(fgmask_history, CV_32FC1); 

	Mat ones = Mat::ones(fgmask_history.size(), CV_32FC1);

	// normalized version of history mask
	Mat fgmask_history_norm = min (ones, fgmask_history / numframes4static); 

	// setting a threshold to history mask to know what stationary fgmask
