...
#include "Fusion.hpp"																		//Header file for GradientTracker
...
int main(int argc, char ** argv)
{
	...
	// Dataset paths
	std::string sequences[] = {"road"};										//test data
	...
	for (int s=0; s<NumSeq; s++ )
	{
		...
		int bins = 8;																				
		int candidates =225;	 																
		char mode = 'F';	// C:color tracker, G:gradient tracker, or F:fusion																		
																									
		//Initialise the object of FusedTracker class with parameters
		FusedTracker Object(mode,list_bbox_gt[0],feat,stride,bins,candidates);
		Rect estimated;
		...
		for (;;) {
			...
			estimated = Object.initiate(frame);											
